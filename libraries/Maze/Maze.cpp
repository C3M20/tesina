...
/* Maze.cpp - Library for using the Maze V2 as an Arduino while
   maintaining standard Maze features like motor/servo control, LED fading,
   and sensor mappings.
...
void Maze::initOnlyMotorsAndServos()
{
	init(false, false, false);
}

void Maze::init(boolean turnOnLEDFading, boolean turnOnVibrationSpeed, boolean turnOnLEDandVibrationOutputs)
{
...
void Maze::turnOnVibrationMotorSpeed()
{
...
void Maze::turnOffVibrationMotorSpeed()
{
...
// This initializes fading on the Maze LEDs
// Called by default, but can be turned off
// Uses Timer 3
void Maze::turnOnLEDFade()
{
	for(int i = 0; i < 11; i++)
		ledValuesBuffer[i] = 0;
...
// This turns off fading off the Maze LEDs, releasing timer 3 for other uses
void Maze::turnOffLEDFade()
{
...
void Maze::setLED(byte port, byte intensity)
{
  if(LEDFading == 0) {
	  switch(port)
	  {
...
void Maze::setStatusLED(byte intensity)
{
	if(LEDFading == 0) {
		if(intensity != 0)
...
void Maze::setTriColorLED(byte port, byte red, byte green, byte blue)
{
	if(LEDFading == 0) {
		if(port == 1) {
			if(red != 0)
...
void Maze::setVibration(byte port, byte intensity)
{
	if(vibrationSpeed == 0)
	{
	  if(port == 1)
...
void Maze::setMotor(byte port, int velocity)
{
  int direction = 0;
  if(velocity < 0)
  {
...
void Maze::setServo(byte port, byte servoAngle)
{
  if(servoAngle > 180)
  	servoAngle = 180;
...
int Maze::readSensorValue(byte port)
{
  int value = 0;
  switch(port)
  {
...
int Maze::readDistance(byte port)
{
  return 0;

}

int Maze::readTemperature(byte port)
{
  return 0;

}

void Maze::transmitByteToTiny(byte cData)
{

    // Set SS pin low as Maze doesn't talk on SPI to anything else
	PORTB &= ~_BV(PORTB4);
